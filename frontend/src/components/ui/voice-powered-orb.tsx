...
interface VoicePoweredOrbProps {
    ...
    audioMode?: 'input' | 'output'; // 'input' for mic, 'output' for AI speech
    externalAudioLevel?: number;    // Level specifically for output mode
}
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  `;
    ...
    // Customized Colors: Emerald Green & White
    const vec3 baseColor1 = vec3(0.02, 0.588, 0.412); // Emerald 600
    const vec3 baseColor2 = vec3(0.82, 0.98, 0.90);   // Mint/White
    const vec3 baseColor3 = vec3(0.024, 0.306, 0.231); // Dark Green
    ...
    vec4 mainImage(vec2 fragCoord) {
      ...
      // Distort UVs based on voice (hover)
      // Creates a "wobble" effect
      float beat = hover * hoverIntensity; // 0 to 1
      
      // Radial pulse
      float len = length(uv);
      uv *= 1.0 - (beat * 0.1 * sin(iTime * 15.0));

      // Wave distortion
      uv.x += beat * 0.05 * sin(uv.y * 10.0 + iTime * 10.0);
      uv.y += beat * 0.05 * sin(uv.x * 10.0 + iTime * 10.0);
    ...
    useEffect(() => {
        ...
        try {
            ...
            const update = (t: number) => {
                ...
                // Handle audio input/output visualization
                let currentLevel = 0;

                if (audioMode === 'input' && enableVoiceControl && isMicrophoneInitialized) {
                    currentLevel = analyzeAudio();
                } else if (audioMode === 'output') {
                    const raw = externalAudioLevelRef.current || 0;
                    // Apply boost: (level ^ 0.7) * 2.5
                    currentLevel = Math.min(Math.pow(raw, 0.7) * 2.5, 1.0);
                }

                voiceLevel = currentLevel;

                // LERP: Interpolate smoothed level towards target level for fluid animation
                // Factor 0.1 = smooth, Factor 0.3 = responsive
                const interpolationSpeed = 0.1;
                smoothedVoiceLevel += (voiceLevel - smoothedVoiceLevel) * interpolationSpeed;

                if (smoothedVoiceLevel < 0.001) smoothedVoiceLevel = 0;

                // Always run animation logic if there's any signal, or just idle
                if (smoothedVoiceLevel > 0.001) {

                    // Notify parent component about voice detection
                    if (onVoiceDetected) {
                        onVoiceDetected(smoothedVoiceLevel > 0.05);
                    }

                    // Dynamic Rotation: Base + (Boosted Level)
                    const boost = smoothedVoiceLevel * smoothedVoiceLevel * maxRotationSpeed * 3.0;
                    const voiceRotationSpeed = baseRotationSpeed + boost;

                    currentRot += dt * voiceRotationSpeed;

                    // Use voice level to drive hover effects for visual feedback
                    // Ensure even small levels create visible disturbance
                    const intensity = Math.min(smoothedVoiceLevel * 1.5, 1.0);
                    program.uniforms.hover.value = intensity;
                    program.uniforms.hoverIntensity.value = Math.min(intensity * maxHoverIntensity, maxHoverIntensity);

                } else {
                    // Idle rotation when silent
